...
def train():
    ...
    for num in test_num:
        for i in range(test_times):
            ...
            filepath = "C:\\Users\\WU-TE\\Desktop\\Manager\\Transfer\\collect_data\\20190125\\collect data"
    ...
    torque_sim = np.zeros((length, 3))   #tao1
    torque_sim_with_f = np.zeros((length, 3))    #tao1+tao2
    torque_sim_only_f = np.zeros((length, 3))    #tao2
    ...
    model.save('dynamic_model_700.h4_2')
...
def test():
    ...
    model = K.models.load_model('dynamic_model_700.h4_2')
    ...
    for i in range(length):
        ...
        torque_after_training[i] = comp   #h4
        # torque_after_training[i] = tc(pos_ori[i, :], vel_ori[i, :], acc_ori[i, :]) + comp   #h2
        # torque_after_training[i] = tcf(pos_ori[i, :], vel_ori[i, :], acc_ori[i, :]) + comp   #h1
